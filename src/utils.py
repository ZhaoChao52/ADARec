...
class EarlyStopping:
    ...
    def save_checkpoint(self, score, model):
        ...
        torch.save(model.cpu().state_dict(), self.checkpoint_path)
        model.to(device) 
...
def get_user_embedding_final(model, input_ids):
    """
    Final, simplified helper function to get user embedding.
    Always uses the 'last' item strategy.
    """
    seq_output = model(input_ids)
    seq_lengths = torch.sum(input_ids > 0, dim=1) - 1
    seq_lengths = seq_lengths.clamp(min=0) 
    return seq_output[torch.arange(seq_output.size(0)), seq_lengths]
