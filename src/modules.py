...
class AdaptiveDepthController(nn.Module):
    def __init__(self, args, hidden_size, num_levels=4):

        super(AdaptiveDepthController, self).__init__()
        self.args = args
        self.num_levels = num_levels
        self.length_embedding = nn.Embedding(513, args.hidden_size)
        self.gru = nn.GRU(
        ...
        self.output_layer = nn.Linear(hidden_size * 2, num_levels)

    def forward(self, sequence_emb, sequence_lengths):

        gru_output, last_hidden = self.gru(sequence_emb)
        sequence_representation = last_hidden.squeeze(0)
        lengths_clipped = torch.clamp(sequence_lengths, max=512)
        length_emb = self.length_embedding(lengths_clipped)
        fused_representation = torch.cat([sequence_representation, length_emb], dim=-1)
        logits = self.output_layer(fused_representation)
